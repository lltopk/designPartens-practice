...
## 适配器模式/第三者模式 ⭐
期望行为 和 待适配类, 之间原来没有任何关系, 现在二者无法集成

你需要创建第三者来实现期望行为接口, 并最终对外暴露带有原适配类逻辑的增强的期望行为方法

对于这个第三者你有两种实现方式
1. 同时继承待适配类和实现期望行为接口, 需要造多个适配器, 但不同的适配器职责单一
2. 只实现期望行为接口, 但是组合所有的待适配类, 只需要一个适配器, 但该适配器逻辑复杂
