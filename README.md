...
- 委托模式: 委托类本身不负责增强实现，它负责找外包干具体的活（被委托者），真正负责增强实现的是被委托类。委托的过程类似于转发forward，因此委托类本身并不复杂。
...
装饰器的功能可能依赖被装饰对象的特性，假如HistorySet需要让set有序，那就可以包装TreeSet，HashSet满足不了需求。

虽然使用继承也可以对被装饰对象进行装饰, 但有的时候组合优于继承：
1. 如果被装饰的对象是final就没办法继承
2. Java只能单一继承，如果你需要继承多个功能类那就做不到了。
3. 装饰器可以多次装饰，层级装饰，你可以发挥想象力各种封装组合，但是继承不可以。

## 适配器模式/第三者模式 ⭐
只有在重构升级老系统的过程中会遇到

期望行为(新的框架统一了接口调用) 和 待适配类(功能不过时但方法名称与期望接口不匹配了), 现在二者无法集成

你需要创建第三者来实现期望行为接口, 并最终对外暴露带有原适配类逻辑的期望行为方法(不涉及增强), 这样老的待适配类也可以加入新框架的统一生态了

对于这个第三者你有两种实现方式
1. 同时继承待适配类和实现期望行为接口, 需要造多个适配器, 好处是不同的适配器职责单一
2. 只实现期望行为接口, 但是组合所有的待适配类, 只需要一个适配器, 但该适配器逻辑复杂, 需要兼容不同的待适配类

对于以上两种实现方式来说, 继承由于组合，代码清晰。

特殊情况是，老的待适配者也是面向接口的，此时组合的优势就体现出来了，组合的成员变量简化为一个老接口就行了，此时的组合实现就变成了委托模式, 但要求适配器实现目标接口来将老的待适配者融入新框架规范。

所以继承 vs 组合孰优孰劣并不能直接下结论, 要结合具体场景分析。
